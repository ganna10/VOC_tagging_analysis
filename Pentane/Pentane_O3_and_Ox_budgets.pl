#! /usr/bin/env perl
# Allocate Ox budgets to initial VOC using same Ox family as in mechanism comparison, compare to previous Ox budget just for pentane
...
$R->run(q` p = ggplot(d, aes(x = Time, y = Rate, fill = Item)) `,
        q` p = p + geom_area(position = "stack") `,
        q` p = p + geom_line(position = "stack", colour = "black") `,
        q` p = p + facet_wrap( ~ Run, scales = "free") `,
...
$R->run(q` CairoPDF(file = "Pentane_O3_and_Ox_allocated_production_budget.pdf", width = 10, height = 7) `,
        q` print(p) `,
        q` dev.off() `,
);
...
sub get_data {
    ...
    for (0..$#$consumers) {
        ...
        my ($reactants, $products) = split / = /, $kpp->reaction_string($reaction);
        $reactants =~ s/_(.*?)\b//g;
        $consumption_rates{$run}{$reactants} += $rate(1:$ntime-2);
    }
    ...
    if ($run eq "Ox") { #allocated HO2x production to VOC
        ...
        for (0..$#$producers) {
            ...
            my ($reactants, $products) = split / = /, $kpp->reaction_string($reaction);
            $reactants =~ s/_(.*?)\b//g;
            $production_rates{"HO2x"}{$reactants} += $rate(1:$ntime-2);
        }
        ...
        for (0..$#$consumers) {
            ...
            my ($reactants, $products) = split / = /, $kpp->reaction_string($reaction);
            $reactants =~ s/_(.*?)\b//g;
            $consumption_rates{"HO2x"}{$reactants} += $rate(1:$ntime-2);
        }
        ...
        foreach my $reaction (keys %{$production_rates{'HO2x'}}) {
            $production_rates{$run}{$reaction} += $production_rates{$run}{'HO2 + NO'} * $production_rates{'HO2x'}{$reaction} / $total_ho2x_production;
            #$consumption_rates{$run}{$reaction} += $consumption_rates{$run}{'HO2 + O3'} * $consumption_rates{'HO2x'}{$reaction} / $total_ho2x_production;
            #$consumption_rates{$run}{$reaction} += $consumption_rates{$run}{'HO2 + NO3'} * $consumption_rates{'HO2x'}{$reaction} / $total_ho2x_production;
        }
        delete $production_rates{$run}{'HO2 + NO'};
        #delete $consumption_rates{$run}{'HO2 + O3'};
        #delete $consumption_rates{$run}{'HO2 + NO3'};
